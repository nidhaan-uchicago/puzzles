...
# the target rewards with 4 different amounts of points, depending on where the dart lands:
...
def score(x, y):
    if x^2 + y^2 <= 1:
        return ("The earned points are 10.")
    else:
        if x^2 + y^2 <= 25:
            return ("The earned points are 5.")
        else:
            if x^2 + y^2 <= 100:
                return ("The earned points are 1.")
            else:
                return ("No earned points.")
